...
/* Reference to variable in Main to allow wake up interval change*/
                    ...
                    bme680_com_fptr_t bus_read, sleep_fct sleep, state_load_fct state_load, config_load_fct config_load)
{
    ...
    static uint8_t bsec_state[BSEC_MAX_PROPERTY_BLOB_SIZE] = {0};
    static uint8_t bsec_config[BSEC_MAX_PROPERTY_BLOB_SIZE] = {0};
    static uint8_t work_buffer[BSEC_MAX_PROPERTY_BLOB_SIZE] = {0};
    ...
    {
        Debug_printf("Failed to Initialize BME680 API %d\r\n",ret.bme680_status);
        await_uart_tx();
        return ret;
    }
    ...
    {
        Debug_printf("Failed to Initialize BSEC Lib %d\r\n",ret.bsec_status);
        await_uart_tx();
        return ret;
    }
    ...
    {       
        ...
        if (ret.bsec_status != BSEC_OK)
        {
            Debug_printf("Failed to load config %d\r\n",ret.bsec_status);
            await_uart_tx();
            return ret;
        }
    }
    ...
    {       
        ...
        if (ret.bsec_status != BSEC_OK)
        {
            Debug_printf("Failed to load state %d\r\n",ret.bsec_status);
            await_uart_tx();
            return ret;
        }
    }
    ...
    if (ret.bsec_status != BSEC_OK)
    {
        Debug_printf("Failed to subscribe to library %d\r\n",ret.bsec_status);
        await_uart_tx();
        return ret;
    }
                    ...
                    state_save_fct state_save, uint32_t save_intvl)
{
    ...
    bsec_library_return_t bsec_control_return;
    ...
    //while (1)
    {
        ...
        if ((bsec_control_return = bsec_sensor_control(time_stamp, &sensor_settings)) != 0)
			Debug_printf("BSEC E=%d\n\r",bsec_control_return);
        ...
        HW_AlarmB_NextAlarmIn((uint16_t)(time_stamp_interval_ms / 1000));

        uint32_t alarmA, alarmB;
        alarmB = HW_get_alarmB();
        alarmA = HW_get_alarmA();
        /* If it can go here, it means that current read is fine. Thus, we compare with the next one */
        if ((alarmB - alarmA) < 6500 || (alarmA - alarmB) < 2500)
        {
            HW_AlarmB_Addup(7000);
        }
        /* 
        if (time_stamp_interval_ms > 0)
        {
            Debug_printf("T:%d #%d - Sleeping for %d\n\r",HW_RTC_Tick2ms(HW_RTC_GetTimerValue()), n_samples, time_stamp_interval_ms);
            await_uart_tx();
            
            
            // sleep((uint32_t)time_stamp_interval_ms);
            // transmit_interval_ms = time_stamp_interval_ms;
        }
         */
    }
}
